(function () {
  ...
  // Small UI toolkit shared by the collector portals: toasts, confirm
  // dialogs, form modals, dropdown menus, pagination and filtering.
  var UI = {};
  ...
  UI.toast = function (message, type) {
    ...
    el.innerHTML =
      ...
      '<div class="font-body-sm text-body-sm text-on-surface flex-1">' +
      window.EcoWasteData.esc(message) + "</div>" +
  ...
  function fieldMarkup(f) {
    ...
    var label =
      '<label class="block font-label-md text-label-md text-on-surface-variant mb-1" for="ui-field-' + f.name + '">' +
  ...
  UI.filterList = function (rows, term, fields, equals) {
    term = (term || "").toString().toLowerCase().trim();
    return rows.filter(function (r) {
      for (var key in (equals || {})) {
        var want = equals[key];
        if (want && r[key] !== want) return false;
      }
      if (!term) return true;
  ...
  // Shared waste-type color coding (chip classes + icon) for both portals.
  UI.wasteMeta = {
    General: { chip: "bg-blue-50 text-blue-700", icon: "recycling" },
    Recyclable: { chip: "bg-emerald-50 text-emerald-700", icon: "recycling" },
    Hazardous: { chip: "bg-red-50 text-red-700", icon: "warning" },
    Organic: { chip: "bg-amber-50 text-amber-700", icon: "eco" }
  };

  UI.wasteType = function (type) {
    return this.wasteMeta[type] || {
